...
| **새 속성** (기존 source 재사용) | `PANEL_COLUMN_META` 에 `kind:"attribute"` + `compute` 키 항목 추가, `ATTRIBUTE_GROUPS` 의 해당 그룹에 키 추가. `generate_attrs.py` 가 자동 인식 — 추가 코드 없음 | `make run` (config 변경 자동 감지) |
| **새 속성** (신규 source) | 위 config 등록 + `generate_attrs._build_schema` 반환 dict에 새 source 키 추가, `main()` 루프에 source별 계산 블록 추가. (예: `"gt_mask"` source — GT 마스크 픽셀 직접 읽기) | `make regen-attr-all` → `make run` |
...
| **새 실험** (새 모델) | `config._EXPERIMENT_LABELS` 에 키 추가 + 예측 SHP를 `source_data/data/pred_shp/<region>/<year>/<exp>/` 에 배치 | `make manifest DS=<ds>` → `make run` |
...
### FiftyOne 데이터셋 생명주기
- `tools/build_manifest.py` 가 patches.sqlite + 항공 TIF + GT/Pred SHP 로 패치 PNG·마스크를 생성하고
  `manifest.json` 을 저장한다. FiftyOne DB 는 이 단계에서 사용되지 않는다.
...
- 시작 시 정리: 오래된 `seg-eval-*` 와 잔여 데이터셋을 삭제해 App 에 예상한 데이터셋만 표시한다.
...
| 새 실험(모델) | pred SHP 배치 후 `make manifest DS=<ds>`, 이후 `make run` |
