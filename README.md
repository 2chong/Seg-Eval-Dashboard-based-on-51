...
| (2) Evaluation | 메트릭 요약(mean/min/max) + 분포 히스토그램 + Confusion Matrix (experiment 별) |
...
→ **[docs/EXTENDING.md](docs/EXTENDING.md)** 참고
...
`main.py`가 시작 시 `sample_attrs.json` / `panel_stats.json` 의 존재 여부와 갱신 시각을 확인해 필요한 단계만 자동 재실행하고 App을 시작합니다.

| 명령 | 언제 사용 |
|------|-----------|
| `make pipeline` | **최초 1회** — 패치 PNG·마스크 생성 + 속성 계산 + 통계 집계 (기본 DS) |
| `make pipeline DS=<name>` | 특정 데이터셋 최초 준비 |
| `make run` | **매 실행** — App 시작 (stale 시 자동 sync 포함) |
...
| `make regen-attr-all` | 속성 compute 코드 변경 후 전 데이터셋 재생성 |
...
+-- Makefile                         # 파이프라인 자동화
+-- pipeline/
|   +-- seg_io.py                    # 마스크·manifest·attrs I/O 공용 헬퍼
...
|   +-- attributes/
|       +-- geometric.py             # GT SHP 기반 기하 속성 20종 계산
|       +-- radiometric.py           # TIF windowed read 기반 방사 속성 4종 계산
+-- tools/
|   +-- generate_attrs.py            # 속성 계산 (config 기반 자동)
...
+-- data_building/
|   +-- <region_year>/               # 데이터셋별 디렉터리 (자동 생성, git 제외)
